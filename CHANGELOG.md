...
## [3.45.1] — 2026-05-07 — Dashboard 全域錯誤通知 Toast 機制 P0 — INotificationService + RuleManagement/AgentSettings try-catch 補齊

統一 Dashboard 前端的錯誤通知機制。**核心 bug 修正**：`UpdateIsActiveAsync` 在 agent 不存在時回傳請求參數而非 DB 值（false-positive 樂觀更新）；`ToggleIsActiveAsync`/`SaveTrustLevelAsync` 缺少 finally 導致 UI 鎖死。**P0 必修**：新建 `INotificationService`（包裝 ISnackbar，統一 Success/Warning/Error 三層）+ 自訂異常類（`RuleNotFoundException`、`AgentConfigurationException`）；`DashboardRuleService` Delete/Toggle 改拋異常取代 silent return；`DashboardAgentService` UpdateIsActive/UpdateTrustLevel 同步修正；`RuleManagement` 補 try-catch + Delete/Toggle 成功/失敗 Toast（Delete 失敗後重載列表確保視覺與 DB 一致）；`AgentSettings` 全面遷移 `_saveMessage` inline alert → Snackbar Toast，補 try-catch + finally。分層規範：Dialog/頁面初始化失敗維持 inline MudAlert，頁面操作結果改 Toast（Success ≈ 3 秒 / Error ≈ 5 秒）。**0 DB migration**（純 UI 層改動）。
