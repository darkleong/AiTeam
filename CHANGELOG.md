...
## [3.46.0] — 2026-05-07 — Dashboard 全域錯誤通知 Toast 機制 Phase 3 — INotificationService + RuleManagement / AgentSettings try-catch 補齊

P0 交付：Dashboard 全域錯誤通知 Toast 機制完整落地。① **INotificationService / NotificationService 建立**（封裝 MudBlazor ISnackbar，ShowSuccess / ShowError / ShowWarning / ShowInfo 四方法，註冊為 Scoped）② **RuleManagement 全面整合 INotificationService**（取代直接 ISnackbar 注入；補齊 OnInitializedAsync / ToggleActiveAsync / DeleteRuleAsync / OpenCreateRuleDialogAsync / OpenEditRuleDialogAsync 五處 try-catch，失敗一律走 ShowError Toast）③ **AgentSettings 全面整合 INotificationService**（取代直接 ISnackbar 注入；新增 ToggleIsActiveAsync / SaveTrustLevelAsync / RestartBotAsync 三處 try-catch with finally 保護 busy flag；既有 SaveProviderModelAsync / SaveTokenLimitsAsync / OnProviderChangedAsync 三處通知呼叫統一改用 INotificationService）。Phase 1/2 基礎已就緒（ISnackbar 既有整合），Phase 3 完成介面抽象層與全頁補齊，P0 Dashboard Toast 通知機制交付完畢。
...
**v4 漸進遷移完整路線 9/9 達成** — Stage 55B Session B 完成 5 routing types HITL refactor（dev_failed_intervention / qa_failed_intervention / devplan_escalate / dev_plan_unable / split_task_proposal）— Pipeline executor 從 SetIntervention end 改 yield-resume + legacy handler 加 Pipeline 分支（議題 5 = 5A 對齊 Stage 55A kickoff/design pattern）。**核心戰略級揭露**：缺口 6 揭露「5 type-specific BossInteraction 在 Pipeline path 下**部分已 fire**（QA / DevPlan / split — by inner service NotifyBoss method），但 Pipeline executor **不 yield 等 Christ** 而是 SetIntervention end → Christ button click 走 legacy handler」 → Session B refactor 策略改為「Pipeline executor 從 SetIntervention end 改 yield-resume + legacy handler 加 Pipeline 分支」（**比預期更輕** — 不需新加 type-specific BossInteraction）。**Session B 6 子項**：① 5 type-specific intervention HITL records（DevInterventionRequest/Response / QaInterventionRequest/Response / DevPlanEscalateRequest/Response / DevPlanUnableRequest/Response / SplitTaskProposalRequest/Response — 含 Action + ModifyContent for split modify）② 5 PortIds + 10 雙向 AddEdge in PipelineWorkflowFactory ③ 4 stage Executor 改 yield-resume routing（DevStage / QaStage / DevPlanStage 含 escalate vs unable 區分 / DesignStage 含 SplitTaskProposal extract BuildEpicSubTasks public method）④ FrameworkPipelineRouter 5 ResumeAfterXxxAsync thin wrappers ⑤ TaskGroupService.ProcessBossResponseAsync 5 case Pipeline 分支（5A 保留 legacy handler 對齊 Stage 55A kickoff/design pattern）+ AppealOrchestrationService / QaCoordinationService 微調對齊 ⑥ InteractionProcessor 路由表補 8 entries + Mock auto-approve switch 補 4 type case + Mock 5 新 scenario alias。**首次拆 Session 戰術完整實踐 + Compact know-how 揭露**：Forge 用 Compact 模式（vs 新開 Forge session）完成 Session A → Compact → Session B — Session A 4 戰略議題拍板脈絡保留 + Session B 不需重新 spike 4 議題，**比新開 Forge session 更乾淨**（沒「兩個 spike 重複成本」+ 對話連貫 + Aria 工作量單線程）。**Stage 55B 整體完整結案** — Session A（v3.43.0 PipelineHitlHelper + 16 處 skip 精簡 + F-α 移除 + 8 處 calling site comment）+ Session B（v3.44.0 5 routing HITL refactor）= **Stage 51 試點 framework HITL pattern 全面 wire 完成**。**Aria 校準錨 Stage 55B 整體 ×1.42**（876K = Session A 450K + Session B 426K vs 中位 615K — **混合型新上界**：拆 Session 但 Compact 戰術讓 know-how 保留，跨 session 加總 context 仍比一個 session 跑 + 1M compact 風險低 trade-off）。**v4 漸進遷移完整路線 9/9 達成 🎉** — 剩 Stage 56 Trial v6 前置條件統包後可進入 Trial_v6 v4 動態架構驗證。詳見 Stage 55B Roadmap。Session B commits：`641594d`(2.1+2.2) + `c5b8853`(2.3) + `7f09af4`(2.4) + `b80f7c9`(2.5) + `9019a2a`(2.6) + `de3b520`(2.7) + `1eb33cc`(2.8) + `315e135`(4) + `77010ad`(7) + `194dff1`(9 結案)。
...
**Stage 55B Session A only** — Session B 5 routing HITL refactor（5 stage Executor + ResumeAfterXxxAsync + Mock 5 新場景，~600-900 LOC）留待 v3.44.0。**4 戰略議題 Christ 拍板**：議題 1 = **1A**（proposal 留 Stage 56 — Forge spike F6 揭露 ProposalConfirmationService 入口流程 group 在 proposal 核准後才 CreateGroupAsync，Pipeline ProposalStage 整合需重構 group lifecycle）/ 議題 2 = **2C**（Pattern A inter-Executor message yield 主 + Stage 51 試點 framework_kickoff_mid_interrupt 獨立保留 — Forge 揭露 Stage 55A KickoffStage/DesignStage 已採 Pattern A，全 type 切 Pattern B 會破壞 Stage 55A ×0.88 已驗 know-how）/ 議題 3 = **3A**（intervention/merge_notify 留 fire-and-forget — ack only no routing 切 yield-resume 收益 = 0，intervention 跨 8 stage AddEdge wiring 規模 L+ 收益 0）/ 議題 4 = **4A**（拆 Session B — Forge spike 揭露 5 routing HITL 規模 ~600-900 LOC，首次拆 session 守「每 Session 規模可控」）。**Session A 範圍 6 子項**：① **PipelineHitlHelper 共用 helper**（議題 2 = 2C 最小共用機制，~50 LOC，符合「3 次再抽象」原則 — 比 Aria 預期 base class 更務實）② **AppealOrchestrationService 11 處 + QaCoordinationService 5 處 = 16 處 skip 全清**（dead code 結論：`UseFrameworkPipeline=true` 唯一 path → `isPipelinePath` 必為 true → 所有 `if (!isPipelinePath) doLegacy()` 永不執行 → 全部刪除；AppealOrchestration 940 → ~700 行 -25% / QaCoordination 237 → ~170 行 -28% / HandleDevBlocker body -65% switch case 全段 dead code 刪）③ **F-α 排除條件 4 處移除**（FrameworkAppealRouter / FrameworkDesignRouter / FrameworkKickoffRouter / MeetingOrchestrationService 各 1 處 `&& g.PipelineFrameworkStateJson == null` 移除 — sub-task 不會有 Kickoff/Design FrameworkStateJson，race risk = 0）④ **8 處 calling site comment 補強**（5 既有不切 type ceo_confirm/ceo_reply/exec_confirm/epic_partial_paused/sage_escalate + intervention/merge_notify 議題 3 = 3A + proposal 議題 1 = 1A）⑤ **Production DB pre-check**（Forge 主動執行：96 done + 54 cancelled + 14 failed + 1 stuck mock NewFeature group，PipelineFrameworkStateJson IS NULL 無 Pipeline path 干擾 → dead code 移除 production safe）⑥ **Version bump v3.42.0 → v3.43.0** + Roadmap 結案第一段（4 戰略議題拍板紀錄 + Session B 留作）。**Aria 校準錨 Session A ×0.73**（450K vs Stage 55B 整體中位 615K，**半 Stage 校準錨計算非典型** — 完整 Stage 55B 校準錨等 Session B 結案後重新評估）。**Stage 55B 整體 8.5/9 達成**（Session B 結案後 9/9 達成 + v4 路線 9/9 達成）。詳見 Stage 55B Roadmap。commits：`6b4c6f9`(主) + `a484ff9`(結案)。
...
**議題 G3 真正解決** — Pipeline 從 Kickoff 階段啟動（Stage 53A 方案 C 留的核心戰略級 TODO）。4 件子工作合一：① **inner FrameworkKickoffRouter / FrameworkDesignRouter 瘦身** — 加 `skipFinalize` 參數 + 改回傳 Outcome record（Forge 主動範圍變更比 plan 純加 bool 更精緻）；既有 entry method 保留作 legacy path（feature flag false 時用）② **Pipeline 主 Workflow 加 KickoffStage / DesignStage Executor + 拓撲擴展**（5→7 RequestPort + 8→10 stage Executor + KickoffCompletion/DesignCompletion port）+ Pipeline Stage Executor 接管 finalize actions（CreateKickoffConfirmationAsync / FinalizeDesignAsync sticks Stage 54 idempotency check）③ **MeetingOrchestrationService.HandleKickoffConfirmedAsync / HandleDesignConfirmedAsync continue/stop 走 Pipeline ResumeAfterKickoff/Design**（modify/restart 沿用 legacy 不在 Pipeline 內處理 re-entry）④ **sub-task 整合**：FireOneStepAsync entry guard 兩入口分流（parent: Kickoff / sub-task: Dev_plan）+ PipelineState.IsSubTask + PipelineStartExecutor 兩出口路由（Stage 46 業務語義保留 — Forge 拍板方案 C 解 Aria 拿捏 #1/#11 內部衝突）⑤ **HandleAgentCompletedAsync 6+1 hooks 全段移除**（含 QA hook，Aria 計劃書漏算 — Forge 補強）⑥ **WorkflowEngine.cs 精簡**（保留 WorkflowType enum + WorkflowStep record；刪 GetDecision + NextAction + WorkflowDecision class — Forge 範圍變更比 plan 整檔刪除更精準，因為 enum + record 跨 service reference）+ TaskGroupService / QaCoordinationService 4 處 caller 移除 + DI 註冊清理。**Forge Plan Mode 主動揭露 3 個 Aria 預掃缺口拍板**：① inner router method 名已存在（Stage 50/52 既有）→ skipFinalize 參數選項 2 ② sub-task first step = `Dev_plan` ≠ `Kickoff`（Aria 拿捏 #1 vs #11 衝突）→ 兩入口分流方案 C ③ EpicChain 不依賴 6 hooks ✅ 好消息（從 `MarkGroupDoneOrInterventionAsync:699` 觸發）。**Aria gate1 揭露 1 critical + 1 minor + 1 nuance**：① critical Mock 場景擴充 + Forge 自驗未做（時序：Forge 不知道剛寫好的 `forge-self-verify` skill）→ Forge 補 4 alias + Dashboard MockScenarioCard + 場景 B/E 自驗全綠 ② minor CS9113 unread parameter `qaCoordination` ctor 移除 ③ nuance modify/restart 沿用 legacy code-level 驗證 OK。**驗收期 follow-up #1（戰略級）**：Forge 自驗場景 E 揭露 **Stage 54 既有遺留 bug** — `InteractionService.cs:132-139` MockMode auto-approve switch 漏 `split_task_proposal` type → split_task_proposal BossInteraction status=responded action=`ack` fall through default → BuildEpicSubTasksAsync 沒被觸發 → Pipeline 卡死 DesignStage SplitProposalOpened 後；修法 commit `492d2db` switch 加一條 `"split_task_proposal" => "split_accept"` — **Forge 自驗能力突破真正生效**（Stage 53B/54 自驗能力進化在 Stage 55A 揭露遺留 bug）。**Christ 視覺驗收 4 張截圖**：場景 B Discord/Dashboard（Pipeline 從 Kickoff 啟動 7 stage 全綠 — 議題 G3 解法視覺證明）+ 場景 E Discord/Dashboard（1 parent + 3 sub-task + sub-task 流程詳情**沒有** Kickoff/Design stage — 缺口 2 兩入口分流戰略級視覺證明）。**Aria 校準錨 ×0.88**（482K vs 中位 545K，混合型第 8 資料點 mid 中段；8 資料點區間穩定 ×0.73-1.25 — 跟 Stage 53B 一樣 ×0.88）。**9 Stage 遷移 8/9 達成**，剩 Stage 55B BossInteraction HITL 推廣（27 處 caller refactor）。詳見 Stage 55A Roadmap。commits：`1cddaef`(主) + `b6e0764`(Aria fix) + `492d2db`(split_task_proposal fix) + `b9365a6`(v2.0)。
