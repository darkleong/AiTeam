...
/// <summary>
/// 根據 Talent 名稱建立對應的 ILlmProvider 實作。
...
/// Stage 87 A2：v4 collapse 最後殘留收口 — agentConfigCache 切 TalentMetaCache / cache key 從 AgentName ("PM") 改 Talent.Name ("Petra")；
/// appsettings fallback path 同步改 `Agents:Petra` key（既有 `Agents:PM` 砍）。
/// </summary>
    ...
    TokenCostEstimator costEstimator)
{
    ...
    /// <summary>
    /// 依 Talent 名稱（baseline："Petra" / "Victoria" / "Cody" 等）建立對應的 Provider，並自動包裝 Token 追蹤與守門。
    /// MockMode 啟用時回傳 MockLlmProvider（不包裝 TokenTrackingProvider）。
    /// Stage 87 A2：Provider / Model 優先讀 DB（TalentMetaCache / talents 表），null 時 fallback appsettings（key = Talent.Name）。
    /// </summary>
    public ILlmProvider Create(string talentName)
    {
        ...
        if (!_settings.Agents.TryGetValue(talentName, out var config))
            throw new InvalidOperationException($"找不到 Talent appsettings 設定：{talentName}（appsettings.json Agents:{talentName} 段缺）");

        // Stage 87 A2：DB override 優先（TalentMetaCache / talents 表），null 欄位 fallback appsettings（per-field）
        var (dbProvider, dbModel, _, _) = talentMetaCache.Get(talentName);
        ...
        var inner = finalProvider.ToUpperInvariant() switch
        {
            ...
            _ => throw new NotSupportedException($"不支援的 LLM Provider：{finalProvider}（Talent={talentName}）")
        };
