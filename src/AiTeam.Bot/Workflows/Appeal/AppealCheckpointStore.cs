...
///
/// 持久化格式（task_groups.FrameworkAppealStateJson）：
/// {
///   "checkpoints": {
...
/// }
/// </summary>
public sealed class AppealCheckpointStore : ICheckpointStore<JsonElement>
{
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<AppealCheckpointStore> _logger;
    ...
    /// <summary>從 DB task_groups.FrameworkAppealStateJson 載回 in-memory（Bot 啟動時 router 呼叫）。</summary>
    public async Task LoadFromDbAsync(Guid groupId, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogInformation("[AppealCheckpointStore] Group={Id} 無 checkpoint 可載入（new session）", groupId);
            return;
        }
        ...
        try
        {
            ...
            _logger.LogInformation(
                "[AppealCheckpointStore] Group={Id} 載入 {Count} 個 checkpoint，latest={Latest}",
                groupId,
        ...
        {
            _logger.LogError(ex,
                "[AppealCheckpointStore] Group={Id} FrameworkAppealStateJson 解析失敗，視同無 checkpoint", groupId);
        }
    }
    ...
    public ValueTask<JsonElement> RetrieveCheckpointAsync(string sessionId, CheckpointInfo key)
    {
            ...
            !dict.TryGetValue(key.CheckpointId, out var value))
        {
            throw new KeyNotFoundException(
                $"[AppealCheckpointStore] sessionId={sessionId} checkpointId={key.CheckpointId} 不存在");
        }
    ...
    private async Task PersistToDbAsync(string sessionId, CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning(
                "[AppealCheckpointStore] sessionId={SessionId} 不是 Guid，無法寫 DB", sessionId);
            return;
        }
        ...
        {
            _logger.LogWarning(
                "[AppealCheckpointStore] Group={Id} 寫 FrameworkAppealStateJson 0 rows affected（group 已被刪除？）",
                groupId);
        }
    }
}
