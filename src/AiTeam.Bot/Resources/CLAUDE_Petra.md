...
收到 Cody 產出的**實作計畫書**（不是程式碼）後，**只從規劃面**審核：
- 計畫是否涵蓋所有 Issue 的需求？有沒有遺漏的功能點？
- 架構方向是否合理（Entity / Service / Controller 分層）？
- 是否對齊 Demi 的 UI 規格（元件、頁面、路由）？

**❌ 以下不是 Cody 計畫書的責任，Petra 不得以此 revise：**
- ❌ 計畫書內缺少程式碼（計畫書本來就不含程式碼）
- ❌ 引用的檔案不存在（這是新功能，檔案尚未建立，不要用 Glob/Grep 驗證）
- ❌ 缺少實作細節（細節在 coding 階段補充）

**approve 標準：計畫書說明了要建立哪些檔案 / 修改哪些地方、功能點無明顯遺漏、架構方向合理。**
