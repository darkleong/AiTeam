...
| v2.0 | 2026-05-14 | **實作 + Aria gate1 通過 + Forge 自驗 Mock 全綠 + Christ 拍板 Stage 65 結案後 merge feature/v5-poc → main**（Trial_v11 在 main 跑驗證）。**Forge 3 commits feature/v5-poc 結構清晰**：`e9dd1ac` 子項 4（CLAUDE_Cody.md +35 純 doc）+ `b536c53` 子項 3（compose entrypoint + Dockerfile gosu install）+ `8115809` 子項 1+2 mega（adapter ritual 移除 + IClaudeCodeService 6 method 加 systemPrompt + ClaudeCodeService BuildArgs conditional flag + ClaudeCodeProxy 連動 + token_logs 移 finally + 7 test 擴），實作順序 4→3→1+2 對齊 plan「風險低到高」。**Forge spike 揭真相品質高**（Stage 58 結論第七次驗證）：① IClaudeCodeService path `src/AiTeam.Bot/Agents/` 不是 Services/（揭 Aria 計劃書沒明寫的細節） ② **adapter ctor tokenLogService production 非 null**（推翻 Aria 預掃 Vera 0 root cause 候選） ③ **adapter try block 範圍揭 Vera 0 真實 root cause #1**（DispatchWithRetryAsync 拋 LlmApiFailureException → token_logs 寫入跳過 — 修法 token_logs 移 finally + capturedException 紀錄）。**Forge healthy 偏離 plan 三處全保 v4 0 regression**：① systemPrompt 放 ct **之後**（vs Aria plan 寫放 ct 之前）— Forge 揭真實 v4 caller `DevAgentService.cs:276` 用 5-positional `RunAsync(localPath, prompt, model, apiKey, cancellationToken)`，systemPrompt 放 ct 之前會 type mismatch compile error → Forge ct 之後對齊 v4 0 break ② ClaudeCodeProxy.cs 連動（Aria + Forge plan 都漏列 — IClaudeCodeService 既有實作類 Stage 17 MockMode runtime switching pattern interface 改必然連動 Forge 自補健康） ③ Dockerfile RUN apt-get install gosu 配套（子項 3 路線 B compose entrypoint gosu exec appuser 必需 — Forge plan 標 fallback 路徑真實做了）。**Aria gate1 走 Tier 0 + Tier 1 第二次實踐**（對齊 Stage 64 同等級 production-ready 收口）：Tier 1 三項深度 catch（新 method 完整讀 ~80 行 GetResponseAsync + try-catch-finally 結構 / Test fixture assertion 抽樣 T1×6 capability + T2 dispatch 拋仍 0 動 + T7 exception 仍 propagate / scope creep 反向交叉檢查 changed files 對齊 plan 兩個 not-in-plan healthy 連動）— 全綠通過。**Forge 自驗 Mock 全綠**：dotnet test AiTeam.Bot.Tests 42/42 + Generated 127/127 全綠 + dotnet build 0 error + 0 v4 regression 守底線（grep 7 worker AgentService caller 0 觸發新 path）。**範圍邊界明寫留 Trial_v11**（Forge 自驗物理限制範疇）：場景 1-2 真實 PR diff 0 CLAUDE.md 改動 + Vera token_logs SQL 驗 / 場景 3 workspace volume 真實 docker volume rm + recreate 驗 / 場景 4 Cody 5/5 範圍 cover 真實任務驗 — 全留 Trial_v11 真實任務驗收。**Stage 65 結案後 merge to main 拍板**（Christ 2026-05-14 — 對齊「production-ready 漸進 path 優先 + Trial_v11 後一次性切 default flag」）：merge 策略 `git merge --no-ff feature/v5-poc`（留完整 commit history + merge commit 標明分支整合時點）。merge 後 main 帶 Stage 63B PoC + Trial_v9 5 fix + Stage 64 + Stage 65 全 commit。Directory.Build.props v3.52.0 → v3.55.0 一次性 bump（跳 v3.53.0/v3.54.0 — main 從未 bump 過直接到 Stage 65 結案版本）。**Aria 校準錨 ×0.79**（297K vs 預估 mid 375K — 跟 Stage 64 ×0.79 連續兩次同倍率穩定 = 紀律累積成熟度進化曲線實證 + production-ready 補強區間 ×0.78-0.99 完整驗證 6 資料點 / 第 7 條延伸範圍段 #3 升級紀律對齊 Aria reference 既有 pattern 憑印象沒 grep caller positional args 順序的同類根因第六次累積修根因）。**Trial_v11 鋪排**：操作前置條件 1 從「切 feature/v5-poc」改為「main 直接 docker compose recreate（self-hosted runner Stage 65 merge 後自動 deploy 完成）+ Dashboard / Internal API 切 feature flag UsePetraOrchestratorV5=true」。**戰略主軸**：Stage 65 Mock 全綠 ✅ → merge to main → Trial_v11 在 main 驗證真實 production-readiness → 通過 → Christ 拍板切 default flag = v5 動態架構正式上線。commits：`d693fa9`(規劃 main) + `e9dd1ac`+`b536c53`+`8115809`(Forge 3 commits feature/v5-poc) + 結案第二段 merge + Directory.Build.props bump (待 main commits)。 |
