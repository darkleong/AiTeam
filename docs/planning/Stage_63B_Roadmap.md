...
| v2.0 | 2026-05-12 | **Forge 結案 + Aria 第二段補強**（Aria）— v3.53.0 deployed 到 feature/v5-poc branch + 9 子項全部完成 + Aria gate1 修正全採納（critical 路線 A + important Victoria 簡化紀錄 + nice-to-have xUnit only 自決紀錄）。**結案 commit `8b5d339`**：37 檔變更 / +2,121 / -206 / dotnet build 0 error / dotnet test **153 passed**（134 baseline + 19 new Petra test — Stage 63A 3 spike test silently pass + Stage 63B 7 xUnit method × 多 InlineData = 26 case total）/ main branch 完全不動 / feature flag default=false v4 production 0 影響。**路線 A 拍板對應實作**：① 限制 (a) workaround — 自寫 PetraOrchestratorService.DecideAsync + BuildSequential + InProcessExecution.RunStreamingAsync events 訂閱（246 行 — 對齊 Stage 63A spike 已驗 path + framework 投資保留）② 限制 (b) workaround — Worker.CreateAgent factory pattern 包 ChatClientAgent + ClaudeCodeChatClientAdapter : IChatClient（98 行 — 7 capability dispatch IClaudeCodeService 對應 method 三層 wrapper 真實生效）。**12 新建檔**：PetraSession + PetraSessionMessage entity + PetraSessionRepository + Migration `20260511163222_Stage63PetraSessionTables`（82+879+85）+ IAgentTool + AgentCapabilityAttribute + PetraSessionContext + ClaudeCodeChatClientAdapter + PetraOrchestratorService + PetraOrchestratorResult + PetraSessionRecoveryService hosted service + PetraOrchestratorServiceTests xUnit 179 行。**修改檔**：AppDbContext +25 / WorkflowSettings +7 / WorkflowSettingsResolver +5 / CeoAgentService +15（flag forward）/ 7 Worker 各 +14-15 / Program.cs +15（DI multi-registration）/ CLAUDE_Petra.md 全砍 -206+255 / Directory.Build.props v3.53.0。**Aria gate1 第二輪通過放行**（v1.1 修正三點全完整：① critical 路線 A 對應 IAgentTool factory pattern + Worker.CreateAgent + PetraOrchestratorService BuildSequential 三處修正 ② important Victoria 簡化紀錄 子項 6 完整段 ③ nice-to-have xUnit only 子項 8 + Test 7 驗 BuildSequential 三層 wrapper）。**兩點 healthy 範圍變更紀錄**（Aria 第二段補對齊紀律）：① **7 partial CLAUDE_*.md 簡化為 FF 五十九 hand-off only（+2 行/檔，Victoria +7）** — 沒做 Plan v1.1 寫的「去除『審核 / 申訴 / 固定 pipeline』字樣 + 補『Petra 動態調度 / Tool Set 介面』段 ~30 行/檔」。戰略上合理（Stage 63B Mock 階段足夠 / Stage 64+ 全量遷移時才必要做完整 partial 重寫對齊 Petra orchestrate production reasoning）— Stage 64+ FF 候選紀錄補完整 partial 重寫 ② **Forge 揭 7 Worker.CreateAgent 重複 abstract 出 PetraWorkerHelper.cs**（49 行新建 — 不在 Plan 9 子項清單）對齊 Stage 36+ 抽 helper SOP 精神（每個 Worker +14-15 行 vs Plan 預估 +30 行 — 抽 helper 後 LoC 減 30% healthy 範圍補強）。**Victoria 簡化範圍紀錄**（Aria gate1 important 2 落實到子項 6）：Stage 63B 階段 = flag check forward + CLAUDE_Victoria.md prompt 移除 codebase scan 段（partial 改動 +7 行）/ 留 Stage 64+ 全量遷移 = 純 facade Router + RouteToPetra Tool Set 完整化 + Charter 02 Layer 2 errata 補釘紀錄 spike 階段 vs production 階段差異。**xUnit only 自決紀錄**（Aria gate1 nice-to-have 3 落實到子項 8）：本 Stage 採 xUnit 7 test method × 多 InlineData = 26 case + InMemory EF Core test 取代 Dashboard MockScenarioCard 5 場景觸發 — Mock 階段紀律對齊 + 0 LLM cost / 5 Dashboard 場景觸發完整化留 Trial_v9 階段（對齊 Stage 56 既有 33 framework_* 場景 pattern — 真實任務階段視覺驗收）。**dotnet test baseline 漂移 134 → 153**（+19 new Petra test — 未來新 Stage 看到 153 應對齊本 v2.0 紀錄理解）。**Aria gate1 commit 檢查通過放行**（8b5d339 對照 Plan v1.1 9 子項全做 + 0 follow-up commits）。**Aria 校準錨 待 Christ 補 Forge context 數字**（規模 L + 架構級新建 + EF Migration + 7 worker + 8 prompt + feature flag / Aria 預估 mid 750K / 對齊 production-ready 補強 4 Stage 區間 ×0.78-0.99 mid 中段 + Stage 63A spike 揭 framework limitation 早期 derisk 範圍可控）。**戰略主軸**：Stage 63B PoC 架構基底 ✅ Mock 全綠 → **Trial_v9 啟動條件達成**（feature/v5-poc branch + 5 向對照 v5/v6/v7/v8/v8-dynamic + 7 驗證項 #4 Crash Recovery + #6 遷移成本量化 真實任務驗證 + LLM cost ~$5-15 預估 — 建議 Christ 儲值至 ≥ $30 buffer 才開跑）。**FF 三十六 status 升「進行中 Charter ✅ + API spike ✅ + PoC ✅ + Trial 待」** = 路線 D 真實任務驗證關鍵實證等 Trial_v9 結案後 Christ 拍板路線 A vs B vs C vs D。commits：`a71931c`(規劃) + `8b5d339`(Forge 結案 main 0 動 / feature/v5-poc) + 本 Aria 第二段 commit。 |
