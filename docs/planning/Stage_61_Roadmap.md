...
| 版本 | 日期 | 內容 |
|---|---|---|
| v2.1 | 2026-05-10 | Christ 視覺驗收 + follow-up fix（Forge）— 場景 6 ✅ PASS（IsEstimated 三件套全綠 ⚠️ icon + Tooltip 文字「部分 token 為估算值（非 CLI 直接回傳）」+ cost「~」前綴，Christ 截圖驗收 Dev/PM 兩 Agent row）+ 場景 7 揭露 follow-up bug → 自診自修（commit `dca1830`）：Stage 46 後端 `GetTaskGroupsPagedAsync` + `GetTaskGroupByIdAsync` 只填 4 epic 欄位但**漏填 SubTasks 集合** → IsEpic 計算欄位永遠 false → Stage 61 加的 📦 Epic UI 不踩。修法：加 `PopulateSubTasksForEpicsAsync` helper 給兩 query 後補抓 sub-task by `ParentGroupId IN (...)`。fix 後 Christ 二次截圖驗收 ✅ PASS — epic 主 group row 顯示 🌳 + 📦 Epic + sub-task count chip / Drawer Epic section 顯示 sub-task 列表 + 暫停 epic 按鈕。**Stage 53B/54 自抓自修精神實踐**：視覺驗收揭露隱性 Stage 46 後端遺留 + Forge 自診修根因 + 不擴 Stage 範圍。剩 1 視覺小瑕疵 follow-up 候選：sub-task 描述欄位 narrow column 斷字垂直排列（功能 OK，視覺略醜，不擴本 Stage）。 |
