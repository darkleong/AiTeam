...
## 當前優先級 Top 5（2026-05-14 v7.97 — Trial_v12 🟢 路線 D 完整實證 → 切 default flag = v5 動態架構正式上線拍板時機到）

| # | FF | 標題 | 狀態 | 為何優先 |
|---|---|---|---|---|
| 1 | **切 default flag** ⭐⭐⭐⭐ | **v5 動態架構正式上線拍板**（Trial_v12 🟢 路線 D 完整實證後 Christ 唯一拍板動作 — SQL UPDATE app_settings SET Value='true' + ReloadCache scope=workflow）| 🔴 **拍板時機到 — 待 Christ 拍板** | Trial_v12 業務品質評估 85-90% + Stage 66 3 議題全完整收口 + 連續 3 Trial 業務級成功（PR #372+#373+#374）+ blind spot 0% + 0 🔴 戰略級新類型維持 = v5 動態架構工程準備 + production-grade 驗證全完成。Christ 拍板後 v4 hierarchical static → v5 dynamic orchestrator 路線 D 戰略遷移正式完成 + 進入 production 觀察期累積真實業務數據 |
| 2 | **三十六** ✅ | v4 動態流程架構 — Phase B（Charter+spike+PoC+Stage 64+65+66 ✅ + Trial_v10 業務級成功 ✅ + Trial_v11 部分成功 ✅ + Trial_v12 🟢 路線 D 完整實證 ✅）| 🟡 **待切 default flag 即可 ✅ close** — Charter+spike+PoC+3 Stage+3 Trial 完整 trail 留 reference | 切 default flag 後 FF 三十六 達成 / 整套 Charter+API spike+PoC+3 production-ready Stage+3 Trial 完整 lifecycle 完成 |
| 3 | **六十一** | v5 PoC → production-ready simplification 補強清單 — 12+ 點（Stage 64 收口 4 點 ✅ / Stage 65 收口 4 點 ✅ / Stage 66 收口 3 點 ✅ / **Trial_v12 揭 1 點 v5 PoC post-confirm flow 收尾未做** + 剩 8+ 點 Stage 67+ 評估）| 🟡 standby — v5 上線後 Stage 67+ 評估動工 | v5 上線後累積 production 真實業務數據再評估 — 不阻擋本次上線（剩餘議題都是 simplification / UI 雜訊範疇 / 不影響業務）|
| 4 | **五十四** | Stage 36 後怪物大檔復發 — 子項 1 TaskGroupService ✅ Stage 59 / 子項 2/3 保留評估 | 🟡 中 — v5 上線後 | v5 上線後再評估（路線 D 採用後 v4 conditional path 規模可能可砍 — 等 production 切 default flag + 觀察一段時間後評估）|
| 5 | （prompt DB 化候選）| Worker system prompt 從檔案搬 DB（CLAUDE_Cody.md 等 8 份）| 🟡 standby — v5 上線後評估 | Christ 2026-05-14 拍板候選 standby — 對齊 Stage 38 Provider/Model 動態化 + Stage 47 DB AppSettings 動態化既有架構演進 / 跨專案 Project 變體 + 改 prompt 不 redeploy / Stage 66 子項 1+3 抽 helper 已留 inject 點 / v5 上線後評估啟動 |

> ⚠️ **戰略主軸**：**Trial_v12 🟢 路線 D 完整實證 ⭐⭐⭐⭐ — v5 動態架構工程準備 + production-grade 驗證全完成 / 切 default flag 拍板時機到**。Trial_v12 業務品質評估 85-90%（vs Trial_v10 75-80%）+ Stage 66 3 議題全完整收口 + 連續 3 Trial 業務級成功 PR 真開 + blind spot 0% 🎉（議題 2 token_logs 真實寫入完整收口 best evidence）+ Vera 真實做 review 揭 1 info 議題 + Cody 範圍對照表段含 InteractionCenter 評估理由 + 順手修 Blazor Server Circuit production bug。**0 🔴 戰略級新類型維持** = v5 設計層仍對。揭 1 🟡 新議題（v5 PoC post-confirm flow 收尾 — Trial_v12 第一個揭露 / 不阻擋上線 / candidate 進 FF 六十一）。**Aria 紀律累積成熟度進化曲線完整實證**：計劃前 WebSearch 紀律連續 3 次（Stage 64/65/66）+ Gate1 Tier 升級紀律 + 自診 source of truth 紀律根因 8 次累積修根因 + Aria 全程自跑 9-step 模板 3 次實踐穩定。**對戰略大重評估的最終實證升級**：① 連續 3 Trial 業務級成功重現 = infinite loop pattern 確認打破 ② v5 設計層 6 Trial 累積驗證 + Stage 66 framework chain 修根因 + 紀律累積成熟度進化曲線 = 路線 D 全套實證資料完整 ③ 切 default flag 後正式進入 v5 動態架構 production 觀察期累積真實業務數據（給未來 prompt DB 化 / FF 六十一其餘 / 全程動態 orchestrator 候選等戰略路徑提供決策資料）。**戰略終局**：Christ 拍板切 `Workflow:UsePetraOrchestratorV5` default true → **v5 動態架構正式上線 🎉**（v4 hierarchical static → v5 dynamic orchestrator 路線 D 戰略遷移完成）。詳見 [Trial_v12_Plan.md](../experiments/Trial_v12_Plan.md) v2.0 結案紀錄。
