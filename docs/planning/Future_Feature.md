...
## 當前優先級 Top 5（2026-05-11 v7.88 — Stage 63A ✅ API spike 完成後 = Stage 63B PoC 啟動條件達成待 Christ 拍板）

| # | FF | 標題 | 狀態 | 為何優先 |
|---|---|---|---|---|
| 1 | （Stage 63B PoC 候選） ⭐ | **Stage 63B PoC spike**（v5 動態架構小範圍真實任務驗證）| 🟡 **啟動條件達成 — 待 Christ 拍板開跑** | Stage 63A ✅ **硬通過 + 揭 2 framework limitation 戰略級早期 derisk**（base GroupChatManager subclass 不啟動 manager loop → 候選 (B) 自寫 orchestrator / base AIAgent subclass 不被 framework dispatch → IChatClient adapter 必走）→ Stage 63B PoC: feature/v5-poc branch + **候選 (B) 自寫 PetraOrchestratorService + BuildSequential** + **ClaudeCodeChatClientAdapter : IChatClient** + 6 子項全量實作（8 CLAUDE_*.md prompt 重寫 + 9 Worker 走 ChatClientAgent + Capability attribute + EF Migration + Trial_v9 5 向對照）/ 規模 L / cost ~600-1000K + LLM cost ~$5-15 / API 餘額 $17.22 建議儲值 ≥ $25 |
| 2 | **三十六** ⭐ | v4 動態流程架構 — Phase B（**Charter ✅ Stage 62 / API spike ✅ 硬通過 Stage 63A / PoC 待 Stage 63B**）| 🟡 進行中 — Charter ✅ + API spike ✅ 硬通過 + PoC 待 | 80% 既有設計拍板 + Stage 62 Charter 4 deliverable + Stage 63A 真打硬通過揭 2 framework limitation（早期 derisk 給 Stage 63B 戰略級價值）+ Stage 63B PoC 啟動條件達成 |
| 3 | **五十四** ⭐ | Stage 36 後怪物大檔復發 — **子項 1 TaskGroupService ✅ Stage 59 / 子項 2/3 保留評估** | 🟡 中 — Stage 63B PoC 後 | Stage 63B PoC 重寫範圍可能自然吸收 ButtonCallbackRouter / DevAgentService 部分 LoC（v5 重寫後哪些大檔仍存在 Stage 63B PoC 後重評估）— 對齊路線 D 才動工 |
| 4 | （Charter+API spike+PoC 後候選） | **戰略大重評估**（AiTeam 是否過度設計 / 路線 B 大砍複雜度 vs 路線 D 繼續 v5）| 🟡 戰略級候選 — Christ 提出 — **Stage 63B PoC 結案後再啟動評估** | Charter + API spike + PoC 三階段 spike 後若 ROI 為負 → 啟動戰略大重評估 / 若 ROI 為正 → v5 全量遷移路線（Stage 64+） |
...
> ⚠️ **戰略主軸**：**Stage 63A ✅ API spike 完成（v3.52.0）— FF 三十六 Phase B 動態決策 API 廉價驗證**。下一階段戰略決策點：① **Christ 視覺驗收 spike notes 5 段 + 觸發 Forge 自驗補填真實 3 場景 LLM 動態決策 log + cost 數字** ② **Stage 63B PoC spike**（feature/v5-poc branch / 6 子項全量實作 / Trial_v9 5 向對照 / 7 驗證項實證 / 規模 L / cost ~600-1000K — 建議儲值 ≥ $25 buffer，餘額 $17.22）③ **PoC 後路線 D 採用 vs 戰略大重評估拍板**。Stage 63A 核心 finding：Charter 候選 `MagenticOrchestrator<TState>` **不存在於 nuget 1.3.0** — 動態決策真實 hook = `GroupChatManager.SelectNextAgentAsync` + `AgentWorkflowBuilder.CreateGroupChatBuilderWith` + `HandoffWorkflowBuilder`（命名落差 ≠ 驗證項 #2 失敗 — Charter 用 errata 補釘到 spike notes 不整篇重寫）。Christ 拍板 EF Migration 路線 **(c) in-memory session**（spike 階段 0 Migration，Stage 63B production-ready 才寫）。詳見 [Stage_63A_Roadmap.md](Stage_63A_Roadmap.md) v2.0 + [v5_charter/05_Stage_63A_Spike_Notes.md](../architecture/v5_charter/05_Stage_63A_Spike_Notes.md)。
