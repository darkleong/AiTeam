...
## 當前優先級 Top 5（2026-05-14 v7.96 — Stage 66 結案後 = v5 動態架構工程準備全完成 → Trial_v12 → 切 default flag = v5 正式上線）

| # | FF | 標題 | 狀態 | 為何優先 |
|---|---|---|---|---|
| 1 | **Trial_v12** ⭐⭐⭐ | **Stage 66 結案後 v5 上線前最後一道閘門**（Aria 全程自跑 9-step 模板第 3 次實踐 — 驗 Stage 66 3 議題收口效果 + 連續 3 Trial 業務級成功 = 路線 D 完整實證）| 🔴 啟動條件達成 — 待 Christ 拍板開跑 | Stage 66 Mock 全綠 + Aria gate1 通過 + Forge 自驗 4 場景 PASS（場景 D 留 Trial_v12 真實任務驗）。沿用 Trial_v6-v11 同 prompt + main 直接驗 / 5-15 min / cost $1-5。驗收：Vera 真實 cover review work（token > 0 + tool role message 寫入）+ PetraSessionMessages tool role 真實寫入（PR body Worker summary 段含完整 chain）+ Cody PR body / commit message 含「範圍對照表」段（5/5 範圍 cover 含 InteractionCenter）|
| 2 | **三十六** ⭐ | v4 動態流程架構 — Phase B（Charter+spike+PoC+Stage 64+Stage 65+Stage 66 ✅ + Trial_v10 業務級成功 ✅ + Trial_v11 部分成功 → Trial_v12 待）| 🟡 進行中 — Trial_v12 ✅ 後切 default flag = v5 正式上線 | 連續 2 Trial 業務級成功（PR #372+#373）+ Stage 66 framework chain 修根因 + 連續 3 次 framework 結論誤判 WebSearch 紀律累積成熟度進化曲線 + 0 🔴 戰略級新類型維持 = 設計層仍對。Trial_v12 通過 → SQL UPDATE app_settings + ReloadCache 切 default true |
| 3 | **五十四** ⭐ | Stage 36 後怪物大檔復發 — 子項 1 TaskGroupService ✅ Stage 59 / 子項 2/3 保留評估 | 🟡 中 — v5 上線後 | v5 上線後再評估（路線 D 採用後 v4 conditional path 規模可能可砍 — 等 production 切 default flag + 觀察一段時間後評估）|
| 4 | （多項保留群組）| FF 二十五 / 四十六 / 四十八（Cody Worker prompt 對齊）| 🟡 保留 — v5 上線後 | v5 上線後 Cody Worker prompt 路徑 = Petra orchestrator dispatch 動態 / 既有對齊紀律可能自然吸收 — 觀察 Trial_v12+v13 模式再決定 |
...
> ⚠️ **戰略主軸**：**Stage 66 結案 ✅ — v5 動態架構工程準備全完成 / Trial_v12 是 v5 上線前最後一道閘門**。Stage 64+65+66 三波 production-ready 補強完成（Stage 64 路線 D 採用拍板首發 + Stage 65 Trial_v10 4 議題收口 + Stage 66 Trial_v11 3 議題收口含 framework chain 修根因 GitHub `microsoft/agent-framework#1308`）。**Aria 紀律累積成熟度進化曲線實證**：① 計劃前 WebSearch 紀律連續第三次實證生效（Stage 64/65/66 三次揭 framework 否定/正面結論誤判同類根因第六次累積修根因）② Aria 校準錨 Stage 66 ×0.58 vs Stage 64+65 連續 ×0.79 -27% 偏低 = production-ready 補強區間 6 → 7 資料點新下界擴展 ×0.58-0.99 / 校準錨偏低四因素疊加（雙重 WebSearch + Forge plan 階段自診揭兩條 Aria 源頭 + 子項合併實作 + 0 critical Aria gate1）③ Aria 自診 source of truth 紀律根因第 7+8 次累積（nuget 版本字串憑印象 + reference method 沒 grep caller dead code）→ workflow_aria.md 第三節 A 第 7 條延伸範圍段 #4 必升級。**Trial_v12 啟動條件達成**：Aria 全程自跑 9-step 模板第 3 次實踐 — 通過 → Christ 拍板切 `Workflow:UsePetraOrchestratorV5` default true = v5 動態架構正式上線。詳見 [Stage_66_Roadmap.md](Stage_66_Roadmap.md) v2.0 + [CHANGELOG v3.56.0](../../CHANGELOG.md#3560)。
