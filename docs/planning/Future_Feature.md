...
## 當前優先級 Top 5（2026-05-13 v7.91 — Trial_v9 結案後 = 戰略大重評估時機到）

| # | FF | 標題 | 狀態 | 為何優先 |
|---|---|---|---|---|
| 1 | （戰略大重評估） ⭐⭐⭐ | **戰略大重評估**（路線 A 繼續 v4 修補 / 路線 B 大砍複雜度 / 路線 C Claude Code 模式 / 路線 D v5 全量遷移）| 🔴 **戰略級時機到 — 待 Christ + Aria 深度討論拍板** | Trial_v9 結案揭關鍵實證 — v5 真實 work（架構不是問題）+ production-ready 工作量 = Stage 64+ 全量遷移大投資（8 prompt 重寫 + framework dispatch protocol 解 + 12+ FF 六十一補強整套） + 連續 4 Trial 揭 18+ 🔴 + 第 4 次 0 deliverable = infinite loop pattern 真實實證 + Christ 既有 2026-05-10 拍板路線 D 後 Stage 62 Charter / Stage 63A spike / Stage 63B PoC / Trial_v9 完整 trail = ROI 評估資料**充分** |
| 2 | **三十六** ⭐ | v4 動態流程架構 — Phase B（**Charter ✅ / API spike ✅ / PoC ✅ / Trial ✅ 揭 12+ 議題 → 待戰略大重評估拍板**）| 🟡 進行中 — 等戰略大重評估拍板才決 Stage 64+ 啟動 vs 廢棄 | Trial_v9 揭「能跑 ≠ 能 deliver」差距 — production-grade 工作量明確 = Stage 64+ 全量遷移範圍 / 路線 D 採用才啟動 / 非路線 D → feature/v5-poc 不 merge + FF 三十六 ✅ close（Charter+spike+PoC+Trial 完整 trail 留 reference）|
| 3 | **六十一** | v5 PoC → production-ready simplification 補強清單（Trial_v9 揭 12+ 議題）| 🟡 standby — Stage 64+ 路線 D 採用才動工 | Trial_v9 揭從 4 點擴 12+ 點完整清單（worker prompt 太簡 / framework dispatch protocol / workspace 持久化 / retry path / etc）— 對齊路線 D Stage 64+ 全量遷移範圍 |
| 4 | **五十四** ⭐ | Stage 36 後怪物大檔復發 — 子項 1 TaskGroupService ✅ Stage 59 / 子項 2/3 保留評估 | 🟡 中 — 戰略大重評估後 | 戰略大重評估拍板路線後再評估動工（路線 A 維持 v4 → 直接動工 / 路線 D → Stage 64+ 自然吸收 / 路線 B/C → 廢棄）|
| 5 | （多項保留群組）| FF 二十五 / 四十六 / 四十八（Cody Worker prompt 對齊）| 🟡 保留 — 戰略大重評估後 | 視戰略路線決定（路線 A → 直接動工 / 路線 D → Stage 64+ 吸收 / 路線 B/C → 重評估）|

> ⚠️ **戰略主軸**：**Trial_v9 結案 ⭐ 戰略級成功 vs 業務級失敗（雙面，第 4 次連續 Trial）— 戰略大重評估時機到**。Trial_v9 跨 2 日 9 PetraSession 累積 5 fix（TurnToken / workingDir / workspace / token_logs / nullable）+ 真實 cost ~$0.63 + 最終 Cody success 但 0 deliverable（Worker prompt 太簡 → reasoning 沒做事 exit success）+ 揭 12+ 議題完整清單（FF 六十一補強範圍）。**戰略級成功**：v5 真實 work 證實 + Anthropic API 5xx confirmed transient + Stage 63A spike「framework limitation (b)」誤判 root cause 揭真實（漏 TurnToken trigger）。**業務級失敗**：「能跑」≠「能 deliver」精準揭差距。**對 Christ 拍板路線 A vs B vs C vs D 的 ROI 評估資料充分**（v5 不是設計問題 / production-ready 工作量 = Stage 64+ 全量遷移大投資 / 連續 4 Trial 揭 18+ 🔴 + 第 4 次 0 deliverable = infinite loop pattern 真實實證）。**Aria 主動提醒戰略大重評估深度討論**（Christ 2026-05-10 拍板對齊精神延續）。詳見 [Trial_v9_Plan.md](../experiments/Trial_v9_Plan.md) v2.0 結案紀錄。
