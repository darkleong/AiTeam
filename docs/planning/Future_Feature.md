...
## 當前優先級 Top 5（2026-05-12 v7.89 — Stage 63B ✅ PoC 完成後 = Trial_v9 啟動條件達成待 Christ 儲值 + 拍板開跑）

| # | FF | 標題 | 狀態 | 為何優先 |
|---|---|---|---|---|
| 1 | （Trial_v9 試驗候選） ⭐ | **Trial_v9 試驗計劃**（v5 動態架構真實任務驗證 + 5 向對照 v5/v6/v7/v8/v8-dynamic）| 🟡 **啟動條件達成 — 待 Christ 儲值 ≥ $30 buffer + 拍板開跑** | Stage 63B PoC ✅ Mock 全綠 + 路線 A 對齊 Stage 63A spike 已驗 path + 2 framework limitation workaround 三層 wrapper 真實生效 → Trial_v9: feature/v5-poc branch + Christ 切 feature flag `Workflow:UsePetraOrchestratorV5=true` + Petra Provider→Gemini Flash + 跑 Trial_v6/v7/v8 同 prompt + 5 向對照 + 7 驗證項 #4 Crash Recovery + #6 遷移成本量化 真實任務驗證 / LLM cost ~$5-15 / 餘額 $17.22 建議儲值至 ≥ $30 buffer 才開跑 |
| 2 | **三十六** ⭐ | v4 動態流程架構 — Phase B（**Charter ✅ Stage 62 / API spike ✅ 硬通過 Stage 63A / PoC ✅ Stage 63B / Trial 待 Trial_v9**）| 🟡 進行中 — Charter ✅ + API spike ✅ + PoC ✅ + Trial 待 | 80% 既有設計拍板 + Stage 62 Charter 4 deliverable + Stage 63A 真打硬通過揭 2 framework limitation + Stage 63B PoC 9 子項 Mock 全綠（路線 A 對齊三層 wrapper）+ Trial_v9 真實任務驗證啟動條件達成 |
| 3 | **五十四** ⭐ | Stage 36 後怪物大檔復發 — **子項 1 TaskGroupService ✅ Stage 59 / 子項 2/3 保留評估** | 🟡 中 — Trial_v9 + 戰略大重評估後 | v5 PoC 走 feature/v5-poc branch 不動 main v4 hierarchical static — ButtonCallbackRouter / DevAgentService 大檔暫保留評估 / 戰略大重評估拍板路線 A vs D 後動工 |
| 4 | （Trial_v9 後候選） | **戰略大重評估**（路線 A 繼續 v4 修補 / 路線 B 大砍複雜度 / 路線 C Claude Code 模式 / 路線 D v5 全量遷移）| 🟡 戰略級候選 — Christ 提出 — **Trial_v9 結案後拍板** | Stage 63B PoC ✅ Mock 全綠 / Trial_v9 = 路線 D 真實 ROI 量化關鍵實證 / Trial_v9 結果 = Christ 拍板路線採用基線（v5 全量遷移 Stage 64+ vs 戰略大砍 vs 維持 v4） |
...
> ⚠️ **戰略主軸**：**Stage 63B ✅ PoC 架構基底 + Mock 全綠（v3.53.0）— FF 三十六 Phase B v5 動態架構 production-ready 實作完成**。下一階段戰略決策點：① **Christ 儲值至 ≥ $30 buffer**（Trial_v9 LLM cost ~$5-15 / 餘額 $17.22 太薄）② **Trial_v9 試驗計劃啟動**（feature/v5-poc branch + 5 向對照 + 7 驗證項真實任務驗證 + 文件對齊 Trial_v2-v8 既有獨立試驗計劃模式 `docs/experiments/Trial_v9_Plan.md`）③ **Trial_v9 結案後 Christ 拍板路線 A vs B vs C vs D 戰略大重評估關鍵實證**。Stage 63B 核心 deliverable：12 新建檔（PetraOrchestratorService + ClaudeCodeChatClientAdapter + IAgentTool factory pattern + 7 entity/service/test + EF Migration Stage63PetraSessionTables + PetraSessionRecoveryService hosted service）+ 7 Worker IAgentTool factory + feature flag default=false v4 production 0 影響。Charter 04 inconsistency 揭露補釘（「9 Worker」實際 7 AgentService / 「RunWriteAsync」實際 `RunAsync`）。dotnet test baseline 漂移 134 → 153（+19 new Petra test）。詳見 [Stage_63B_Roadmap.md](Stage_63B_Roadmap.md) v2.0 + [CHANGELOG v3.53.0](../../CHANGELOG.md#3530)。
