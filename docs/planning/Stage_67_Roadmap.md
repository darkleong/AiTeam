...
| v2.0 | 2026-05-15 | **Stage 67 結案 ✅ — v5.5 升級首發 Phase 1 Step 2 完成 / Trial_v13 啟動條件達成**。**Forge 一氣呵成 4 議題 spike + 8 子項 1 commit `58ed302`**：21 檔（修改 11 + 新建 5 + 搬移 2 + Migration 2 autogen + Directory.Build.props v3.56.0→v3.57.0 minor bump） / 1871 insertions / 27 deletions。**Plan Mode Spike 揭 4 議題 Aria 全 endorse**：① Petra 是 Scoped 不是 Singleton（Program.cs:98 grep 揭）→ IServiceScopeFactory pattern 不需要 / 設計決策 9 刪除 ② xUnit baseline 真實 13 [Fact]/[Theory]（Roadmap 28 誤 propagate from Stage 66）→ 校正 13 → 17+ case（Theory 展開後 dotnet test count 32 PASS）③ 路線 B endorse（保留 IAgentTool 演進為 ITalent）— Aria WebSearch 二次確認 framework AddAIAgent register pattern 對「1 agent 1 capability」場景設計 / AiTeam Talent 兼多 Skill 超出 framework 預設範圍 / 0 新 NuGet / xUnit Mock 0 重做 / 對齊「修根因 > 補丁」+「production-ready 漸進 path 優先」精神 ④ 7 worker class **不 archive** 保留 v5 既有 fallback path（Aria Roadmap 子項 3 規劃內部矛盾揭 — archive class 違反 Christ 決議 4 fallback 紀律）/ 只 archive **2 CLAUDE_X.md**（Rosa / Demi 真實存在 / Release / Maya 不存在）。**Forge healthy 偏離 plan 3 處全 endorse**：① 新建 ITalentFactory.cs（取代 plan 早期 IEnumerable<ITalent> DI scan / 解 app.Build 時 DB 還沒 ready 的時序雷 + 副作用 Phase 3 dynamic CRUD 自然解 — DefaultTalentFactory 用 IServiceScopeFactory 對齊 plan 戰略脈絡精神 Singleton 內 query DB scoped）② FinalizeGitAsync 簽名變更 IAgentTool picks → string dispatchNames（v5 worker name 與 v5.5 talent name 共用統一傳）③ BuildPetraSystemPrompt 加「需求拆解紀律」段位置（對齊既有四段式風格 trigger / 判準 / 範例 / 紀律 — 對齊 Aria nice-to-have 提醒）。**Aria 二診 4 條 nice-to-have 對齊**：CLAUDE_Cody.md 兼任職務紀律段 ✓ 對齊（ui_design 引用 archive/CLAUDE_Demi.md / release_publishing 從零自定義 0 引用 archive/CLAUDE_Release.md）+ 子項 3 register 邏輯 ✓ Forge 走 ITalentFactory pattern 完全 bypass plan A/B 方案討論（更乾淨）+ 2 條 Roadmap 文字 polish（子項 5 標題「砍 4 份」+ 關鍵檔案清單殘留 archive/CLAUDE_Release.md）留 v1.0 不追溯改 / v2.0 結案紀錄揭真實對齊 Stage 65/66 既有紀律。**Aria gate1 Tier 0+1+2+Tier 3 #11 全綠 0 critical 修正項**：dotnet build 0 Error / 102 baseline warning（0 新引入）/ dotnet test 178 PASS（51 AiTeam.Bot.Tests + 127 AiTeam.Tests.Generated — v4 既有 0 regression 對齊驗收場景 G）/ PetraOrchestratorServiceTests 32 PASS（13 baseline [Theory 展開] + 4 新 Test14-17）/ 5 新檔完整讀（GenericAgentTool / ITalent / ITalentFactory / Skills/ISkillRegistry / SkillDescriptor）/ WebSearch 結論反向 cross-check ✓ 0 framework limitation 漏接。**Aria 自診揭 source of truth 紀律根因 5+1 = 第 9-14 次累積**（plan review 階段 5 條 + gate1 階段 1 條延伸）：① 第 9 次 Petra Singleton 假設沒 grep Program.cs ② 第 10 次 xUnit baseline 28 沒 grep PetraOrchestratorServiceTests.cs ③ 第 11 次 AddAIAgent framework pattern 沒驗 AiTeam 真實場景（Talent 兼多 Skill / capability 動態 dispatch 超出 framework 預設）④ 第 12 次 4 CLAUDE_X.md archive 沒 grep Resources/ 真實檔案 ⑤ 第 13 次 7 worker class archive vs Christ 決議 4 內部矛盾（規劃邏輯漏掃）⑥ 第 14 次 Petra Scoped 修正後沒延伸到 Talent factory lifecycle（DefaultTalentFactory Singleton 內 query DB scoped 仍要 IServiceScopeFactory — Forge spike 自診揭）→ workflow_aria.md 第三節 A 第 7 條延伸範圍段 **#5 立**（5 類同類根因 Stage 67 一次揭 + 第 14 次延伸 — 升級為「規劃前必查 DI lifecycle / xUnit baseline / framework pattern 對齊真實場景 / archive 來源檔真實存在 / fallback 紀律 vs archive 動作交叉檢查」5 類紀律）。**Aria 校準錨 ×0.58**（Forge 真實 409K vs 預估 mid 700K — **跟 Stage 66 完全相同 ×0.58 連續兩次同倍率穩定** = **架構級重構新區間 ×0.58 立**）。校準錨穩定四因素：① 計劃前 + plan reviewer 雙重 WebSearch 揭真實 framework 限制 ② Forge plan 階段自診揭 5 條 Aria 源頭省 implementation iteration ③ Forge healthy 偏離 plan 3 處全 endorse 0 返工 ④ Christ 5 條決議 + Phase 1 Step 1 Baseline 拍板提供完整設計脈絡 0 拆解 spike。**production-ready 補強區間 ×0.78-0.99 + 架構級重構新區間 ×0.58 兩層分層成立**（前者 Stage 56/58/60/61/64/65 6 資料點 / 後者 Stage 66/67 2 資料點）— 對未來 v5.5 Phase 2 Step 3-5 規劃預估提供指引。**結案第二段 step 0 升級**：① workflow_aria.md 第三節 A 第 7 條延伸範圍段 **#5 立**（5 類同類根因紀律 — Stage 67 一次揭 5 條 + gate1 揭 1 條延伸） ② calibration_anchors.md Stage 67 校準錨段 + **架構級重構新區間 ×0.58** 連兩次實證 ③ 校準錨速查表 加 Stage 67。**Trial_v13 啟動條件達成**：Stage 67 Mock 全綠 + Aria gate1 通過 + Forge 自驗 7 場景 cover by xUnit Test14-17 + 178 全 PASS / 場景 C v5.5 path 真實 task 留 Trial_v13 真實任務驗。**戰略主軸**：v5.5 Phase 1 Step 2 完成（Talent-Skill separation 重構基底架構級升級成功）→ Trial_v13 是 v5.5 Phase 1 完整收口前最後一道閘門 → 通過 → Christ 拍板切 `Workflow:UseTalentSkillSeparation` default true → v5.5 Phase 1 完成 + 進 Phase 2 Step 3 DB 持久記憶 schema 設計。 |
