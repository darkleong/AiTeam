...
| 版本 | 日期 | 內容 |
|---|---|---|
| v1.0 | 2026-05-10 | 初版計劃書建立（Aria）— Stage 62 = FF 三十六 Phase B Charter spike（v5 動態架構規劃文件 deliverable）。**戰略脈絡**：Trial_v6/v7/v8 連續揭 6 🔴 + deliver 度持續倒退 = infinite loop pattern 真實實證 = v4 hierarchical static 補強 ROI 為負 → Christ 2026-05-10 拍板路線 D（FF 三十六 Phase B 動態架構 spike — 對齊 6 個月前 brainstorm 既有 roadmap 第 4 選項）。**Stage 62 = Charter spike + Stage 63 = PoC spike 兩階段拆分**（對齊 Stage 51 spike Charter 模式 + Stage 53A 拆 Stage 模式）— Charter 通過才 commit PoC 投資。**範圍 7 子項**：① 補完 7 驗證項細節（Victoria Router / Petra 自主調度 / per-task session / Crash Recovery / Mock Gemini Flash / 遷移成本量化 / Hybrid 會議 trigger）② 架構 wire 規劃（4 層 Hierarchy 落具體 service / DI 結構）③ v4 code audit 盤點（保留 / 重寫 / 吸收三類分類 + LoC 量化）④ PoC spike Roadmap 草稿（Stage 63 範圍 + 規模 + 觸發條件）⑤ 戰略大重評估後 backlog 大整理（FF 五十九/六十/五十七/五十八 close + FF 二十五/四十六/四十八 保留 + FF 五十四子項 2/3 重評估）⑥ Charter spike 揭露議題追蹤 ⑦ v3.51.0 bump。**設計決策 8 條 Christ 全拍板**：Charter 文件 only / Charter main + PoC branch / 保留 v4 不動 / 保留 10 Agent / 同 prompt 任務 / Forge spike healthy 模式 / Stage 51 spike Charter 模板 / minor bump。**規劃前期已 grep 驗證**：FF 三十六既有 80% 設計拍板（4 層 Hierarchy + 5 挑戰 + Hybrid 會議 + Tool Set + Capability-based）+ 7 驗證項清單 + Worker pool 10 Agent service 既有位置 + Stage 51 既有 spike Charter 模式 + Stage 53A 拆 Stage 模式。**規模 M** / Opus 1M + medium / 預估 ~300-400K（純文件 deliverable + 盤點 + 規劃 + Forge spike 揭露議題自決多項細節）。**Charter spike 純文件 deliverable** — 不動 production code / 不動 EF Migration / Stage 60+61 既有 11 Mock 場景 + dotnet test 131 passed 0 regression 自動對齊。 |
